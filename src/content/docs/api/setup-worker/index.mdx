...
When called without any arguments, the function will return you a worker instance that has no network description (i.e. no requets handlers). Use the methods like [`worker.use()`](/docs/api/setup-worker/use) and [`worker.resetHandlers()`](/docs/api/setup-worker/reset-handlers) to add and remove _runtime_ request handlers to the worker instance.
