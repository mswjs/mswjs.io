...
However, the `HttpResponse` class enables certain features, like mocking response cookies, which are not available when using the standard `Response` class. For the sake of consistency, we highly recommend using the `HttpResponse` class on a daily basis. We acknowledge that it is a library-specific API and we are decicated to make it teach you about the web fundamentals through respecting the default `Response` constructor signature and methods as well as minimizing the amount of library-specific features this class ships.
