...
It's common to enable the interception in the `beforeAll` hook of your testing framework. Below yuo can find an example of how to do that with Jest.
...
- _Predefined stategy_ or _Custom strategy_ (default: `"warn"`).
...
| ------------- | ---------------------------------------------- |
| `"bypass"`    | B                                              |
| `"warn"`      | Print a warning but perform the request as-is. |
