...
The `graphql` handler namespace no longer gets special treatment. Instead, you should declare standard JSON responses directly.
...
The `request` argument of the `onUnhandledRequest` has changed from being an abstract request object to being a Fetch API `Request` instance. Take that into account when accessing its properties, like `request.url`.
...
This release brings changes to the [Life-cycle events](/api/life-cycle-events) listeners' call signature.
...
Earlier versions of Node.js, like v18.8.0, didn't have official support for `request.formData()`. Please upgrade to the latest Node.js 18.x where such support has been added.
