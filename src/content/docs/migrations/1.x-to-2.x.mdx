...
### `Request`/`Response` is not defined

**Cause:** Your code runs in the environment that doesn't support the global Fetch API.

**Solution:** There are a couple of things you can do to resolve this.
...
Make sure the global Fetch API classes are available in your environment. Some third-party tools, like Jest, intentionally strip Node.js globals when executing your code. If that's the case, make sure to explicitly add those globals back using the appropriate means provided by those third-party tools.

Here's an example of how to do that in Jest:
