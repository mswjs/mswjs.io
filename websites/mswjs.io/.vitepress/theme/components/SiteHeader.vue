...
const documentationFramed = computed(() => {
  return DOCUMENTATION_ROOTS.some((rootPath) => {
...
/**
 * The navigation follows the library being read: "Docs" leads to its
 * documentation root, and MSW's other sections (Guides, API) only show
 * while reading MSW. The blog is the site's and always shows.
 */
const navigationItems = computed(() => {
  const items = (theme.value.nav ?? []).filter(
    (item): item is DefaultTheme.NavItemWithLink => {
      return 'link' in item
    },
  )

  if (library.value.slug === null) {
    return items
  }

  return items.flatMap((item) => {
    if (item.text === 'Docs') {
      return [{ ...item, link: library.value.url, activeMatch: `^${library.value.url}` }]
    }
    ...
    <div :class="{ 'msw-container home-frame': !documentationFramed }">
      ...
      >
        <div class="flex h-full shrink-0 items-center gap-8">
          <a
            href="/"
            class="site-header-logo flex shrink-0 items-center"
