...
The `worker.resetHandlers()` method can be called without any arguments. When done so, all the [Runtime request handlers](/guides/best-practices/network-behavior-overrides) (those prepended via `worker.use()`) will be removed.
