...
You can implement a custom request predicate using a [higher-order resolver](/guides/recipes/higher-order-resolver) function. In the example below, we will create a `withSearchParams` function that will help us match requests that satisfy a custom `predicate` based on the request's query parameters.
