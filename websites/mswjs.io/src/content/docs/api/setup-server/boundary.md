...
> In test runners that support `aroundEach()` (for example, Vitest), you can run each test within a server boundary automatically:
>
> ```js
> aroundEach((runTest) => server.boundary(runTest)())
> ```
>
> This can be a simpler alternative to wrapping each test with `server.boundary()` manually.
