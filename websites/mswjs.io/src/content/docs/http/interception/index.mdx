...
<PageCard
  icon={ServerIcon}
  url="/docs/http/requests/query-parameters"
...
- `:foo` (parameter), to match a named parameter in the URL (see [Path parameters](/docs/http/requests/path-parameters)).
...
| `params`  | `Record<string, string \| string[]>` | Request [path parameters](/docs/http/requests/path-parameters) (e.g. `:userId`). |
| `cookies` | `Record<string, string>`             | Parsed [request cookies](/docs/http/requests/cookies).                           |
