...
export const test = testBase.extend({
  worker: [
    async ({}, use) => {
      // Start the worker before any test starts.
      await worker.start()

      // Expose the worker object on the test's context.
      await use(worker)

      // Stop the worker after all tests have finished.
      worker.stop()
    },
    // Scoped to worker so it's run just once per worker.
  ...
  workerCleanup: [
    ...
    // Scoped to test so it's run once per test.
