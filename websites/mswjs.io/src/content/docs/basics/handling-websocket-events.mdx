...
MSW enables custom logging for both mocked and original WebSocket connections **in the browser** to allow you to:
...
### Message colors and styles

All the printed messages are color-graded by the following criteria: <span className="inline-block h-3 w-3 bg-blue-500 rounded-full" /> system events, <span className="inline-block h-3 w-3 bg-red-500 rounded-full" /> outgoing
messages, <span className="inline-block h-3 w-3 bg-green-500 rounded-full" /> incoming
messages, and <span className="inline-block h-3 w-3 bg-orange rounded-full" /> mocked
events. In addition to the colors, some messages are represented by solid (↑↓) or dotted icons (⇡⇣), indicating whether the event occurred in your application or in the event handler, respectively.
...
```txt /▼ timestamp/ /▲ icon/ /▼ sent data/ /▼ data length/
      ▼ timestamp    ▼ sent data      ▼ data length (bytes)
...
#### <span className="text-red-500">↑</span> Outgoing client message
...
#### <span className="text-orange">⇡</span> Outgoing mocked client message
...
#### <span className="text-green-500">↓</span> Incoming client message
...
#### <span className="text-orange">⇣</span> Incoming mocked client message
...
#### <span className="text-green-500">⇣</span> Incoming server message
...
1. It is common to send stringified payload over the WebSocket protocol, which implies the need of parsing it anyway.
