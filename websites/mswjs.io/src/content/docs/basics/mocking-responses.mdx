...
You don't need to import the `Response` class because it's a part of the global Fetch API in the browser and modern versions of Node.js (v18+). The library supports any variations of the `Response` instances, including shorthand responses created via methods like `Response.json()` or `Response.error()`.
...
<div className="md:grid md:grid-cols-2 md:gap-5">
  ...
  <PageCard
    icon={NewspaperIcon}
    url="/docs/best-practices/typescript"
    title="Using with TypeScript"
    description="Embrace type-safe API mocking with TypeScript."
  />
</div>
