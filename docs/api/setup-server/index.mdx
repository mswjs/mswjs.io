...
<Hint>
  This function is designed for NodeJS environoment. If looking for a way to apply API mocking in a browser environment, consider using <Link to="/sdg"><code>setupWorker</code></Link> instead.
</Hint>
...
> Although there is "server" in `setupServer`, the library does not establish any actual servers, but operates by augmenting native request issuing modules (such as `https` or `XMLHttpRequest`). The namespace is chosen for what the logic _represents_, not how it operates internally.
