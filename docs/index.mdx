...
Modern software development is often done in parallel, which means the deadline is in few days, and waiting for the backend to be finished is not an option. Most likely you are using some sort of mocking already: static fixtures, hard-coded state, or a third-party libraries.
