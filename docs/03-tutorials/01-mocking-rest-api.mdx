...
Describe which endpoints to mock by providing respective calls of `rest[METHOD]` as the arguments to `composeMocks()` function.
...
composeMocks(rest.get('/user/:userId', null))
```
...
The result of calling the `composeMocks()` function is a public API object that contains the `start()` method. This method registers and activates the Service Worker responsible for requests interception and mocking.

Call the `start()` function returned from the previous `composeMocks()` invocation.
