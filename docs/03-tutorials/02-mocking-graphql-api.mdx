...
Mock Service Worker comes with a first-class support for mocking a GraphQL API. This tutorials assumes you have completed the [Getting started](/docs/tutorials/getting-started) tutorial, and have the `msw` package installed in your project.
...
- `setupWorker()`, a function that describes which operations to mock;
...
```js showLineNumbers focusedLines=14-21,32-46
// src/mocks.js
import { setupWorker, graphql } from 'msw'
...
The result of calling the `setupWorker()` function is a public API object that contains the `start()` function. We need to call that function in order for MSW to start the Service Worker responsible for intercepting requests and mocking their responses.
