...
import { PageLink } from '../../src/components/mdx/PageLink'
...
Let's start by creating a mocking definition file called `mocks.js` and import two essential functions from the `msw` package:
...
The second argument to a request handler function is a _response resolver_—a function that determines how to resolve a response. That response resolver function accepts three arguments:
...
First, let's start with the `rest.get()` request handler. Its response resolver returns the call to the `res` function, which describes how to mock the response. In our case we pass two calls to context utilities as the arguments to that `res` function call:
