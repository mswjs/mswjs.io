...
**Response** is a function that composes a mocked response object.

Unlike the `Response` object in NodeJS, the `res` argument in Mock Service Worker is used to create a functional composition chain that passes a newly created mocked response object from one [Response transformer](/docs/basics/response-transformer) to another, eventually returning a ready mocked response to the Service Worker.
