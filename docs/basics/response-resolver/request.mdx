...
<Hint mode="warning">
The <code>req</code> object of the same response resolve will be different in browser and Node environment. Certain request properties are not available in Node (i.e. <code>credentials</code>, <code>integrity</code> or <code>destination</code>). The values of such properties are set to the default values, when referenced in Node environment.
</Hint>
