...
When nothing is returned from a response resolver, MSW performed a captured request as-is, hitting the actual endpoint returning the response. Leverage this behavior to perform conditional mocking, for example based on the request's payload.
