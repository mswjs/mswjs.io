...
> Note that relative URL are resolved against the current location (`window.location`).
...
You can test how Mock Service Worker resolves paths against the actual request URL using the REPL below.
