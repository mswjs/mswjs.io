...
The `graphql` namespace is designed for handling GraphQL operations. It contains a set of [Request handlers](/docs/basics/request-handler) for mocking GraphQL requests.
...
Using a method under this namespace automatically creates a request handler for the respective GraphQL operation. Pass the operation name as the first argument and the [Response resolver](/docs/basics/response-resolver) as the second.
